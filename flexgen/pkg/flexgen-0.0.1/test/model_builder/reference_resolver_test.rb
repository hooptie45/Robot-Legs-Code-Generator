...
require 'flexgen/model_builder/reference_resolver'
...
class ReferenceResolverTest < Test::Unit::TestCase
  ...
  def createJob(hash)
    ...
    FlexGen::ModelBuilder::ReferenceResolver::ResolverJob.new(
  ...
  def test_resolve_same_namespace
    ...
    resolver = FlexGen::ModelBuilder::ReferenceResolver.new
  ...
  def test_resolve_faulty
    ...
    resolver.addJob(createJob(
      ...
      :namespace => b1,
      :string => "b1.c5"))
    assert_raise FlexGen::ModelBuilder::ReferenceResolver::ResolverException do
      resolver.resolve(toplevelNamespace)
    end
  end
