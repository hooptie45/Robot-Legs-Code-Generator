...
private
	...
	def self.typeMap
		{ :EString => String,
			:EInt => Integer,
			:EFloat => Float,
			:EBoolean => FlexGen::MetamodelBuilder::DataTypes::Boolean,
		  :EJavaObject => Object,
		  :ERubyObject => Object,
		  :EJavaClass => Class,
		  :ERubyClass => Class }
...
private
	...
	def setupType(type)
		if self.class.typeMap.keys.include?(type)
			...
			@impl_type = type
		elsif type.is_a?(FlexGen::MetamodelBuilder::DataTypes::Enum)
			@etype = :EEnumerable
			@impl_type = type
    else
...
private
	...
	def setupType(type)
		if type.is_a?(Class) && descendent?(type, FlexGen::MetamodelBuilder::MMBase)
			@etype = nil
			@impl_type = type
    else
