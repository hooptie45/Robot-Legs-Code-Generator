...
require 'erb'
require 'fileutils'
require 'flexgen/template_language/output_handler'
require 'flexgen/template_language/template_helper'
    ...
    class TemplateContainer
      ...
      def _define(template, params={}, &block)
        ...
        if params.has_key?(:any)
      		raise StandardError.new("define: :any must be an Array") \
        		unless params[:any].is_a?(Array)
        	params[:any].each do |a|
        	   @templates[template][a] = TemplateDesc.new(block, params[:local])
        	end
        else
