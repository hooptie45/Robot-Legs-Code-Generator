require 'flexgen/instantiator/nodebased_xml_instantiator'
...
class DefaultXMLInstantiator < NodebasedXMLInstantiator
  ...
  def class_name(str)
	...
	def build_class(name, mod)
		mod.const_set(name, Class.new(FlexGen::MetamodelBuilder::MMBase))
	end
  ...
  def method_name(str)
	...
	def build_p2c_assoc(parent, child, method_name)
		...
		child.object.class.has_one("parent", FlexGen::MetamodelBuilder::MMBase)
	end
