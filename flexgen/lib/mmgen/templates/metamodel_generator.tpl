...
<% define 'Attribute', :for => EAttribute do |rootp| %>
	...
	<% end %><%nows%>
	<% for p in FlexGen::MetamodelBuilder::AttributeDescription.propertySet %>
		<% unless p == :name || p == :upperBound || FlexGen::MetamodelBuilder::AttributeDescription.default_value(p) == getGeneric(p) %>
	    	, :<%=p%> => <%nows%>
...
<% define 'Reference', :for => EReference do |cmd, rootpackage| %>
	...
	<% if eOpposite %><%nows%>
		, '<%= eOpposite.name%>'<%nows%>
	<% end %><%nows%>
	<% pset = FlexGen::MetamodelBuilder::ReferenceDescription.propertySet.reject{|p| p == :name || p == :upperBound || p == :containment} %>
	<% for p in pset.reject{|p| FlexGen::MetamodelBuilder::ReferenceDescription.default_value(p) == getGeneric(p)} %>
    	, :<%=p%> => <%=getGeneric(p)%><%nows%>
	<% end %>
	<% if eOpposite %>
		<% for p in pset.reject{|p| FlexGen::MetamodelBuilder::ReferenceDescription.default_value(p) == eOpposite.getGeneric(p)} %>
...
<% define 'ClassHeader', :for => EClass do |rootp| %>
	...
	<% if eSuperTypes.size > 1 %><% nows %>
		FlexGen::MetamodelBuilder::MMMultiple(<%= eSuperTypes.collect{|t| t.qualifiedClassifierNameIfRequired(rootp)}.join(', ') %>)
	<% elsif eSuperTypes.size > 0 %><% nows %>
		<%= eSuperTypes.first.qualifiedClassifierNameIfRequired(rootp) %>
	<% else %><% nows %>
		FlexGen::MetamodelBuilder::MMBase
	<% end %>
<% end %>
...
<% define 'ClassHeaderFullyQualified', :for => EClass do |rootp| %>
	...
	<% if eSuperTypes.size > 1 %><% nows %>
		FlexGen::MetamodelBuilder::MMMultiple(<%= eSuperTypes.collect{|t| t.qualifiedClassifierName(rootp)}.join(', ') %>)
	<% elsif eSuperTypes.size > 0 %><% nows %>
